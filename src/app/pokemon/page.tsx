...
const Page = () => {
  ...
  useEffect(() => {
    const fetchPokemon = async () => {
      ...
      if (!apiUrl) {
        console.error("POKEAPI_URL environment variable is not defined.");
        setPokemon([]);
        setLoading(false);
        return;
      }
      ...
      try {
        const response = await fetch(`${apiUrl}pokemon/?limit=20&offset=${(pageNumber - 1) * 20}`);
        if (!response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50">
      <div className="container mx-auto py-8 px-4">
        ...
        <Card className="mb-2 shadow-xl gap-1 mb-5 bg-white/90 backdrop-blur-sm border-0 ring-1 ring-gray-200">
          <CardContent className="p-6 py-1">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-gray-100">
              ...
              <div className="text-sm text-gray-600 bg-gray-50 px-3 py-2 rounded-lg">
                Showing {((pageNumber - 1) * 20) + 1} - {Math.min(pageNumber * 20, totalCount)} of {totalCount} Pokémon
              </div>
            </div>
          </CardContent>
        </Card>
